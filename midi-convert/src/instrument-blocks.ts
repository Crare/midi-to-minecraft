...
const instrumentByPatchId: Record<number, string> = {};

function applyDefaultNamespaceToValue([key, block]: [number, string]): [
  number,
  string,
] {
...
function applyDefaultNamespaceToKey([key, value]: [string, string]): [
  string,
  string,
] {
...
export const instrumentByBlock = new Map(
  ...
  ).map(applyDefaultNamespaceToKey)
);
...
export const blockByPatchId = new Map(
  ...
  ).map(applyDefaultNamespaceToValue)
);
