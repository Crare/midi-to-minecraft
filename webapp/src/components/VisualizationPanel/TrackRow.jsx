...
function getNoteblockTooltip(placement) {
  ...
  return [
    ...
    tuningInfo
      ? `Playsound pitch: ${tuningInfo.playsoundPitch.toFixed(6)}`
      : null,
...
function TrackRow({
  ...
  const totalUnits =
    noteLayout.length > 0 ? noteLayout[noteLayout.length - 1].unitEnd : 0;
  ...
  const visEnd =
    scrollLeft +
    (Number.isFinite(containerWidth) ? containerWidth : 1e9) +
    overscan;
  ...
  const beforeSpacerWidth =
    beforeUnitCount > 0 ? beforeUnitCount * CELL - 2 : 0;
  ...
  return (
    <div className={rowClassName}>
      <div className="track-line">
        {beforeSpacerWidth > 0 && (
          <div
            style={{ width: `${beforeSpacerWidth}px`, flexShrink: 0 }}
            aria-hidden="true"
          />
        )}
        {noteLayout.slice(startIdx, endIdx).map((layout, i) => {
          ...
          const spacerWidth =
            layout.spacerUnits > 0 ? layout.spacerUnits * CELL - 2 : 0;

          return (
            <Fragment key={`frag-${noteIndex}`}>
              {spacerWidth > 0 && (
                <div
                  style={{ width: `${spacerWidth}px`, flexShrink: 0 }}
                  aria-hidden="true"
                />
              )}
              <div className="note-unit">
                {note.placements.map((placement, pi) => {
                  const tuningInfo = placement.pitch
                    ...
                    : null;
                  const tooltipLines =
                    getNoteblockTooltip(placement).split("\n");
                  return (
                    <div
                      key={pi}
                    ...
                    >
                      ...
                      >
                        {tooltipLines.map((line, lineIndex) => (
                          <Fragment
                            key={`${noteIndex}-${pi}-tooltip-${lineIndex}`}
                          >
                            {lineIndex > 0 ? <br /> : null}
                            {line}
                          </Fragment>
        ...
        {afterSpacerWidth > 0 && (
          <div
            style={{ width: `${afterSpacerWidth}px`, flexShrink: 0 }}
            aria-hidden="true"
          />
        )}
      </div>
